...
CREATE INDEX idx_tasks_is_archived ON public.tasks(is_archived) WHERE is_archived = false;
...
DECLARE
  auto_rec RECORD;
BEGIN
  -- Only fire when column_id actually changes
  IF OLD.column_id IS NOT DISTINCT FROM NEW.column_id THEN
    RETURN NEW;
  END IF;

  -- Reset is_done and is_archived when moving to a new column
  -- (they will be re-set below if the new column has the automation)
  NEW.is_done := false;
...
-- Also handle INSERT (when task is created directly in a column with automation)
